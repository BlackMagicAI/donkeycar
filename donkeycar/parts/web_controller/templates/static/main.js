...
joystickLoopRunning=true;
        joystickLoop();
		        ...
		        drag: function() {				        	
		    	  	...
		    	  	var knobWidth = $(this).width;
		    	  	var knobHeight = $(this).height;
		    	  	...
		    	  	powerLevel = (knobPosRadius/getJoyStickPadSize().xRadius); //(knobPosRadius/getJoyStickPadSize().xRadius)*100;
				...
				//console.log(data)
				state.tele.user.angle = angle //Math.max(Math.min(Math.cos(radian)/70*distance, 1), -1)
				state.tele.user.throttle = limitedThrottle(powerLevel) //limitedThrottle(Math.max(Math.min(Math.sin(radian)/70*distance , 1), -1))
